...
class AmberSecretsTest {
    @Test fun nativeAdapterCompletesProtectedStorageContractsAndLeavesRestartEvidence() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val intent = Intent(context, MainActivity::class.java).putExtra(MainActivity.EXTRA_APP_SLUG, "secrets-contract")
        val scenario = ActivityScenario.launch<MainActivity>(intent)
        try {
            val device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation())
            assertTrue(device.wait(Until.hasObject(By.text("Secrets contract passed: 82 checks")), 30000L))
        ...
        InstrumentationRegistry.getInstrumentation().runOnMainSync {
            ...
            assertNotNull(CrystalBridge.renderStudy(themed, "secrets-close"))
        ...
        assertEquals("Terminal close did not drain secret operations", Pair(0, 0), pending)
        InstrumentationRegistry.getInstrumentation().runOnMainSync {
            assertEquals(CrystalBridge.NativeDebugCounts(0, 0), CrystalBridge.debugCounts())
        }
    }
