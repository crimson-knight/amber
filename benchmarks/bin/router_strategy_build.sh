...
while (($#)); do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: benchmarks/bin/router_strategy_build.sh --compiler crystal|acrystal|/absolute/path --output /absolute/path [--no-release]

Builds the router strategy benchmark with the normalized Amber macOS toolchain.
EOF
      exit 0
      ;;
    *)
...
build_args+=("-o" "${output_path}" "benchmarks/router_strategy_compare.cr")
