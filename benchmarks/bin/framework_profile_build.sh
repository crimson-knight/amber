...
while (($#)); do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: benchmarks/bin/framework_profile_build.sh --compiler crystal|acrystal|/absolute/path --output /absolute/path [--no-release]

Builds the framework performance profile harness with the normalized Amber benchmark toolchain.
EOF
      exit 0
      ;;
    *)
...
build_args+=("-o" "${output_path}" "benchmarks/framework_performance_profile.cr")
